---
name: "php-moderno"
description: Regole vincolanti e riferimenti per scrivere, rivedere e rifattorizzare codice PHP moderno (8.4+) — tipi stretti, funzionalità 8.x, sicurezza, errori ed eccezioni, PDO, dependency injection, standard PSR, strumenti (Composer, PHPStan, PHP-CS-Fixer/Pint, PHPUnit/Pest, Xdebug 3). Usala ogni volta che scrivi o modifichi codice PHP, anche in progetti Laravel o Symfony, quando fai code review o security review di codice PHP, aggiorni codice legacy PHP 5/7, configuri strumenti di qualità, o avvii un'applicazione PHP senza framework (scheletro PSR-7/15 verificato incluso). Use for any PHP coding, review, refactoring, upgrade or framework-less bootstrap task.
---

# PHP moderno

Claude conosce già PHP. Questa skill serve a due cose: fissare **scelte vincolanti** dove esistono più strade, e **correggere abitudini superate** che circolano ancora in tutorial e codice legacy (incluse fonti autorevoli ma datate come *PHP The Right Way*).
...
2. **Con un framework, il framework decide** architettura, DI, HTTP, ORM, validazione, configurazione, i18n, gestione eccezioni e stile (es. Laravel → Pint, Eloquent, Form Request, `config()`, service provider). Questa skill copre ciò che il framework lascia al codice: linguaggio, tipi, eccezioni di dominio, sicurezza a livello di codice, strumenti. Non introdurre pattern di questa skill che contraddicono le convenzioni del progetto.
3. **Laravel:** per il confine tra Action e Service usa la skill `laravel-action-vs-service` se presente. Non applicare `readonly`, property hooks o costruttori con promozione ai modelli Eloquent: gli attributi sono magici e gestiti dal framework.
4. **Senza framework:** usa lo stack in `references/stack-http-psr.md` e parti dallo scheletro in `assets/skeleton/`.
   **Hosting condiviso senza SSH o senza poter cambiare la document root:** usa la skill `php-hosting-condiviso`, se presente. Ha struttura, `.htaccess`, deploy via FTP e migrazioni via cron già verificati.
5. **Convenzioni del repository** (strumenti configurati, stile, struttura cartelle) prevalgono sulle preferenze di questa skill quando sono esplicite e coerenti.
...
- Stile PER-CS (o il preset del progetto, es. Pint in Laravel), applicato dal fixer e non a mano. Nomi dei simboli in inglese.
...
|---|---|
| `references/linguaggio-8x.md` | scegliere costrutti, verificare in che versione esiste una funzionalità, property hooks, visibilità asimmetrica, novità 8.4/8.5 |
| `references/sicurezza.md` | input, output, SQL, password, crittografia, upload, sessioni, file, security review |
| `references/errori-eccezioni.md` | gerarchie di eccezioni, error handler, logging, cosa catturare e dove |
| `references/database-pdo.md` | PDO senza ORM, opzioni di connessione, transazioni, query dinamiche, `utf8mb4` |
| `references/dependency-injection.md` | container, autowiring, composition root, cosa iniettare |
| `references/stack-http-psr.md` | applicazione senza framework, PSR-7/11/15/17, middleware, emitter, runtime persistenti |
| `references/strumenti.md` | Composer, PHP-CS-Fixer/Pint, PHPStan/Larastan, Rector, PHPUnit/Pest, Xdebug 3, OPcache |
| `references/testo-date-i18n.md` | UTF-8, `mb_*`, `intl`, date e fusi orari, traduzioni |

## Scheletro senza framework

`assets/skeleton/` è un'applicazione minima funzionante (PHP 8.4, PHP-DI, Relay, FastRoute, Diactoros, laminas-httphandlerrunner, Monolog), con test PHPUnit 13, PHPStan livello max e PHP-CS-Fixer PER-CS 3.0 già configurati e verificati. Per un nuovo progetto senza framework copia la cartella, rinomina `name` in `composer.json` e il namespace `App\` se serve, poi `composer install && composer check`. Il `README.md` dello scheletro descrive il flusso di una richiesta e come aggiungere rotte.
